...
const useTitleObserver = (
  ...
  useEffect(() => {
    ...
    // 제목 태그들을 다 찾아낸다.
    const headingElements = textRef.current
      ? Array.from(textRef.current.querySelectorAll('h1, h2, h3, h4, h5'))
      : [];

    // set id of title element
    headingElements.map((elem, idx) => {
      elem.setAttribute('id', md[idx].key);
      return 0;
    });
    ...
    const callback: IntersectionObserverCallback = headings => {
      ...
      headingElementsRef.current = headings.reduce(
        (map: any, headingElement) => {
          const tempMap = map;
          tempMap[headingElement.target.id] = headingElement;
          return tempMap;
        },
        headingElementsRef.current
      );
